...
int* dijkstra2(struct Graph* graph, int src)
{
...
int main()
{
  ...
  for(int i=0; i<E; i++){
    ...
    addEdge(graph2, A-1, B-1, C);
  }
	
  //cout << "Dijkstra Source 1"<< endl;
	int* dist_o; 
  dist_o = dijkstra(graph, 0);

  //cout << "Dijkstra Source N"<< endl;
	int* dist_n; 
  dist_n = dijkstra2(graph, V-1);
  ...
  //iterate through every v
  for(int i=0; i<V; i++){
    AdjListNode* v = graph->array[i].head;
    while(v!=NULL){
      int a = *(dist_o+i);
      int b = *(dist_n+(v->dest));
      if(a+b < total){
